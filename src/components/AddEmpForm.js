...
function AddEmpForm() {
  ...
  const handleEmpAddDetail = () => {
    if (formEmpId !== "" && formName !== "" && formDOJ!=="") {
      dispatch(addEmp({ empID: formEmpId, name: formName, doj: moment((formDOJ).toString()).format("DD/MM/YYYY") }))
    }
    else
  ...
  return (
    <>
      <MyButton style={{ marginLeft: "50px", fontFamily: "cursive", color: "white" }} size='small' variant='contained'
